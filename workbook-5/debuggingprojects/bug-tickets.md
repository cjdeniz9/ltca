...
**Description:**
The MovieTest is failing when checking the borrow duration for movies. Movies should have a 3-day borrowing period, but `getBorrowDuration()` is returning the wrong value — likely because the method body doesn't match what its comment promises.
...
**Actual Behavior:**
Movies are returning an incorrect borrow duration value (the movie's runtime in minutes, not the borrow period in days).

**Hint:** Compare what `getBorrowDuration()` actually returns to the value its inline comment describes. Don't confuse the movie's *length* with how long a patron can *keep* it.
